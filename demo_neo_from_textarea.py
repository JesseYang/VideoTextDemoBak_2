...
def detect_table(inputs, pred_func, enlarge_ratio=1.1):
    def preprocess(inputs):
...
def new_extract_lines(self, inputs, ori_coors):
    ...
    for img_idx, each_input in enumerate(inputs):
        ...
        for each_contour in contours:
            ...
            if (x + w) <= (ori_coors[img_idx][0] + 15) or x >= (ori_coors[img_idx][2] - 15) or (y + h) <= (ori_coors[img_idx][1] + 20) or y >= (ori_coors[img_idx][3] - 20):
                continue
            ...
            if len(self.output_detect_table[img_idx][1]) > 0:
                ...
                for table_coor in self.output_detect_table[img_idx][1]:
                    ...
                    if x  >= (table_coor[1]['detect_table'][0]-5) and (x+w) <= (table_coor[1]['detect_table'][2]+5) and \
                    y >= (table_coor[1]['detect_table'][1]-5) and (y+h) <= (table_coor[1]['detect_table'][3]+5):
                        # cv2.rectangle(img,
...
class Extractor():
    ...
    def _extract_lines(self):
        ...
        pure_outputs, error_idx_lists = new_extract_lines(self,inputs, ori_coors)
        print("error_idx_lists",error_idx_lists)
            ...
            for idx, data in enumerate(group):
                ...
                if len(self.output_detect_table[table_count][1]) > 0:
                    for table_coor in self.output_detect_table[table_count][1]:
                        cv2.rectangle(img,
                        (table_coor[1]['detect_table'][0]+self.output_detect_text_area[table_count][1]['detect_area'][0], table_coor[1]['detect_table'][1]+self.output_detect_text_area[table_count][1]['detect_area'][1]),
                        (table_coor[1]['detect_table'][2]+self.output_detect_text_area[table_count][1]['detect_area'][0], table_coor[1]['detect_table'][3]+self.output_detect_text_area[table_count][1]['detect_area'][1]),
                        (255, 0, 145),
                        3)
