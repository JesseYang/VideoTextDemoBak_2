    ...
    def postprocess(predictions, img, det_th=None):
        ...
        for klass, k_boxes in nms_boxes.items():
            for box_idx, each_box in enumerate(k_boxes):
                ...
                output.extend([cropped_img, {'detect_area': det_area, 'type': klass, 'raw_img': img, 'conf': conf, 'predicted_coor':predicted_coors}])
...
def new_extract_lines(inputs, ori_coors):
    def savitzky_golay(y, window_size, order, deriv=0, rate=1):
...
class Extractor():
    ...
    def _detect_text_area(self):
        ...
        print("after detect_text_area ", len(pure_outputs))
        outputs = []
    ...
    def _extract_lines(self):
        ...
        pure_outputs, error_idx_lists = new_extract_lines(inputs, ori_coors)
